...
//	public Optional<PlateAppearance> getlastPAByGameId(Integer gameId);
	...
	@Query("SELECT P FROM PlateAppearance P WHERE P.game.id = ?1 ORDER BY P.id DESC")
	public List<PlateAppearance> getPAsByGameIdDesc(Integer gameId);
	
	@Query("SELECT P FROM PlateAppearance P WHERE P.game.id = ?1")
	public List<PlateAppearance> getPAsByGameId(Integer gameId);
	...
	@Query("SELECT P FROM PlateAppearance P WHERE P.base != -1 AND P.base != 4 AND P.base IS NOT NULL AND P.game.id = ?1 ORDER BY P.base")
	public List<PlateAppearance> getPAsOnBase(Integer gameId);
	
	@Query("SELECT P FROM PlateAppearance P WHERE P.base != -1 AND P.base != 4 AND P.base IS NOT NULL AND P.game.id = ?1 ORDER BY P.base DESC")
	public List<PlateAppearance> getPAsOnBaseDesc(Integer gameId);
}
